...
- Run Router.js
- Side note: Program defaults to view from current month, So if running a few amount of days, remember to go back to January
...
*   **`p2/`**: Root project directory.
    ...
    *   **`src/`**: Contains server-side source code.
        *   `json/`:
            ...
            *   `rooms.json`: Stores the hotel room data (generated by `roomGenerator.js`).
        *   `scripts/`: Core algorithmic logic.
            *   `algorithm.js`: Implements the preference optimization logic used on "Least Difference" and "Stay Duration" strategies.
            *   `assignBookings.js`: Handles the initial assignment of bookings to rooms based on the selected strategy (Best Fit, Sort by Duration or Random).
            *   `availabilityMatrix.js`: Manages the data structure (`availabilityGrid`) that tracks room availability over time.
            *   `roomGenerator.js`: Generates the `rooms.json` file based on defined parameters in globalVariables.
...
1.  `server.js` starts an HTTP server which can run on a provided server or on `localhost:3000`.
...
3.  `router.js` handles `/reset`:
    ...
    *   Resets `startValue` (used in `allocate` to track simulation progress) and `globalState.currentDay`.
